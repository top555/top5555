...
<!--тут будет вывод самой статистики-->
		Статистика сайта.
<? 
$colv = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-24*60*60")); 
$colv1 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-48*60*60"));
$colv2 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-72*60*60")); 
$colv3 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-96*60*60")); 
$colv4 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-120*60*60")); 
$colv5 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-124*60*60")); 
$colv6 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-148*60*60")); 
$colv7 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-172*60*60")); 
$colv8 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-196*60*60")); 
$colv9 = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(sum) from sp where time > UNIX_TIMESTAMP()-220*60*60")); 
...
$(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            categories: ["<? echo $t9.'","'.$t8.'","'.$t7.'","'.$t6.'","'.$t5.'","'.$t4.'","'.$t3.'","'.$t2.'","'.$t1.'","'.$t.'"'; ?>]
        },
        yAxis: {
            title: {
                text: 'Руб.'
        ...
        series: [{
            name: 'Выплачено',
            data: [<? echo $s9.','.$s8.','.$s7.','.$s6.','.$s5.','.$s4.','.$s3.','.$s2.','.$s1.','.$s;?>]
        }]
    });
});
</script>
